...
// 观察2：对于一个结果中的横坐标x，右边缘 <= x的建筑不会影响到x对应的纵坐标。因此遍历到一个位置x时，为了确定x对应的纵坐标，需要遍历 (左边缘 <= x < 右边缘) 的建筑，选择这些建筑中最高的。复杂度为O(n^2)
// 为了降低复杂度，使用优先队列来寻找符合条件的最高建筑。由于输入数组的左边缘有序，右边缘无序，因此左边缘不符合要求的直接跳过，之后也用再考虑。左边缘符合要求的都先加入队列，之后再判断右边缘是否符合要求
class Solution {
